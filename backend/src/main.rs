...
use rocket::{Rocket,Build, fairing::AdHoc, http::{ContentType, Status, CookieJar, Cookie}, response::status::{Created, NoContent, Unauthorized, BadRequest}, serde::json::Json, request::{FromRequest, self, Request}, outcome::IntoOutcome};
...
impl<'r> FromRequest<'r> for AdminUser {
    ...
    async fn from_request(request: &'r rocket::Request<'_>) -> request::Outcome<Self, Self::Error> {
        request.cookies().get_private("auth")
            .and_then(|c| c.value().parse().ok())
            .map(|c| AdminUser(c)).or_forward(())
    }
}
...
async fn create_cooler(conn: DbConn, cooler: Json<NewCooler>, _admin: AdminUser) -> Result<Created<()>> {
...
fn rocket() -> _ {
    rocket::build()
        ...
        .mount("/", routes![index_cpu, create_cpu, edit_cpu, delete_cpu, index_gpu, create_gpu, edit_gpu, delete_gpu,
            ...
            index_cooler, create_cooler, edit_cooler, delete_cooler, index_case, create_case, edit_case, delete_case, login, logout])
}
